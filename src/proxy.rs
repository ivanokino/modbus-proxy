use std::{convert::TryInto, io::{Read, Write}, net::{TcpListener, TcpStream}};
...

{
    ...
    let mut write_to_host_stream: TcpStream = TcpStream::connect("127.0.0.1:8081")?;
    
    for for_stream in listener.incoming()
    {

        let mut stream: TcpStream = for_stream?;
        let buff: Vec<u8> = handle(&mut stream)?;
        let is_normal: bool = check(&buff);
...
fn check(mut buff: &[u8]) -> bool
    {

    let bytes: [u8; 2] = buff[4..6].try_into().expect("error in the check()");
    let lenght: u16 = u16::from_be_bytes(bytes);

    if (lenght as usize) != (buff.len() - 6) || (buff[2]!=0 && buff[3] != 0) || buff.len() < 7 || buff.len() > 260{
        return false;
    }
